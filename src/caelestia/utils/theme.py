...
@log_exception
def apply_neovim(colours: dict[str, str], mode: str) -> None:
    lua_lines = [
        "-- Auto-generated by Caelestia Theme Engine",
        "return {",
        f'  mode = "{mode}",',
        "  colours = {",
    ]
    for name, colour in colours.items():
        lua_lines.append(f'    {name} = "#{colour}",')
    lua_lines.append("  },")
    lua_lines.append("}")
    atomic_write(c_state_dir / "nvim-colors.lua", "\n".join(lua_lines) + "\n")
